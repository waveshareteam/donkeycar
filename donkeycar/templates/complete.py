...
def add_drivetrain(V, cfg):
    ...
    if (not cfg.DONKEY_GYM) and cfg.DRIVE_TRAIN_TYPE != "MOCK":
        ...
        if cfg.DRIVE_TRAIN_TYPE == "PWM_STEERING_THROTTLE":
            ...
            dt = cfg.PWM_STEERING_THROTTLE_WS
            steering_controller = PCA9685(dt["PWM_STEERING_PIN"], dt["STEERING_ADDR"], busnum=cfg.PCA9685_I2C_BUSNUM)
            steering = PWMSteering(controller=steering_controller,
                                            left_pulse=dt["STEERING_LEFT_PWM"],
                                            right_pulse=dt["STEERING_RIGHT_PWM"])

            throttle_controller = PCA9685(dt["PWM_THROTTLE_PIN"], dt["THROTTLE_ADDR"], busnum=cfg.PCA9685_I2C_BUSNUM)
